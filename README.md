<h2 align="center">👋 Hi, I'm Rajit Chadar</h2>
<h3 align="center">Full Stack Developer (MERN) | B.Tech CSE Student</h3>

<p align="center">
  <img src="https://img.shields.io/badge/Location-Sagar,%20Madhya%20Pradesh-FF2E63?style=flat-square&logo=google-maps&logoColor=white" alt="Location" />
  <a href="mailto:rajitathiya123@gmail.com"><img src="https://img.shields.io/badge/Email-rajitathiya123@gmail.com-blue?style=flat-square&logo=gmail&logoColor=white" alt="Email" /></a>
  <img src="https://img.shields.io/badge/Phone-+91%207489140554-success?style=flat-square&logo=whatsapp&logoColor=white" alt="Phone" />
</p>

---

<br/>

## 🚀 Professional Summary

> Full Stack Developer and B.Tech Computer Science student with strong hands-on experience in **MERN Stack development**, **RESTful APIs**, and **real-time web applications** using **Socket.io** and **WebRTC**. 
> Strong foundation in **Data Structures & Algorithms**, backend system design, and cloud deployment. 
> 🎯 Actively seeking **Software Engineer / Full Stack Developer Internship or Entry-Level roles**.

<br/>

## 🎓 Education

🎓 **Bachelor of Technology (B.Tech) – Computer Science** *Jabalpur Engineering College, Madhya Pradesh* 📅 2023 – 2027 | 📊 **CGPA: 8.61**

🏫 **Class XII – MP Board (2023)** — **90.4%** 🏫 **Class X – MP Board (2021)** — **94%**

---

## 🧠 Technical Skills

### 💻 Programming Languages
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=flat&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)

---

### 🌐 Web Technologies
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=flat&logo=express&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![REST API](https://img.shields.io/badge/REST-API-blue?style=flat)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat&logo=tailwindcss&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?style=flat&logo=bootstrap&logoColor=white)
![Material UI](https://img.shields.io/badge/Material_UI-007FFF?style=flat&logo=mui&logoColor=white)
![Socket.io](https://img.shields.io/badge/Socket.io-black?style=flat&logo=socket.io)
![WebRTC](https://img.shields.io/badge/WebRTC-333333?style=flat&logo=webrtc)

---

### ⚙️ DevOps & CI/CD
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)
![CI/CD](https://img.shields.io/badge/CI%2FCD-GitHub_Actions-2088FF?style=flat&logo=githubactions&logoColor=white)

---

### 🗄️ Databases
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)

---

### 🛠️ Tools & Platforms
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)
![AWS S3](https://img.shields.io/badge/AWS_S3-569A31?style=flat&logo=amazonaws&logoColor=white)
![Render](https://img.shields.io/badge/Render-000000?style=flat&logo=render&logoColor=white)

---

### 📚 Core Computer Science
![DSA](https://img.shields.io/badge/Data_Structures_&_Algorithms-important?style=flat)
![OOPs](https://img.shields.io/badge/OOPs-blueviolet?style=flat)
![DBMS](https://img.shields.io/badge/DBMS-success?style=flat)
![OS](https://img.shields.io/badge/Operating_Systems-informational?style=flat)
![CN](https://img.shields.io/badge/Computer_Networks-yellow?style=flat)

---


## 🚀 Featured Projects

### 🥇 1. Bargit – Hyperlocal E-Commerce & Dynamic Negotiation Platform
**Tech Stack:** React 18, Vite, Node.js, Express.js, MongoDB Atlas, Material UI  
*A comprehensive 3-tier marketplace bridging the gap between offline local retailers and digital consumers.*
- Engineered a real-time **Dynamic Negotiation Engine** allowing customers and vendors to bargain live over product MRPs.
- Implemented **Geospatial routing** utilizing MongoDB's `2dsphere` indexes (`$near` queries) for highly accurate location-based store discovery.
- Developed a highly secure **Role-Based Access Control (RBAC)** architecture separating B2C (Customer), B2B (Vendor ERP), and Admin ecosystems.

💻 **GitHub:** [rajitchadar59/bargit-nearby-bargain-marketplace](https://github.com/rajitchadar59/bargit-bargaining-marketplace)

---

### 🥈 2. MyGit – Custom Git System & GitHub Clone
**Tech Stack:** Node.js, React.js, Express.js, MongoDB, AWS S3, Socket.io  
*A fully functional version control CLI tool paired with a GitHub-like web platform.*
- Built a **custom Git CLI** supporting core commands (`init`, `add`, `commit`, `push`) from scratch.
- Implemented real **version control logic** with commit history and unique tracking IDs.
- Stored file versions and blobs securely utilizing **AWS S3** buckets.
- Developed a comprehensive web platform featuring user profiles, repositories, and issue tracking.

🔗 **Live:** [my-github-z2q3.onrender.com](https://my-github-z2q3.onrender.com)  
💻 **GitHub:** [rajitchadar59/my_github](https://github.com/rajitchadar59/my_github)

---

### 🥉 3. My Video Call – Real-Time Video Conferencing Platform
**Tech Stack:** React.js, Node.js, Express.js, MongoDB, WebRTC, Socket.io  
*A seamless, low-latency communication platform for audio/video meetings.*
- Integrated **WebRTC** for peer-to-peer, real-time video and audio streaming.
- Developed instant messaging and connection signaling pipelines utilizing **Socket.io**.
- Added advanced features including Screen Sharing and meeting history logs.
- Engineered a **"Join as Guest"** feature enabling frictionless access via meeting links without requiring authentication.

🔗 **Live:** [my-video-call-frontend-dzg0.onrender.com](https://my-video-call-frontend-dzg0.onrender.com)  
💻 **GitHub:** [rajitchadar59/my-video-call](https://github.com/rajitchadar59/my-video-call)

<br/>

## 🏆 Certifications & Achievements

- 🥇 Certified in **Data Structures and Algorithms with Java**
- 🏅 Full Stack Web Development (MERN) Certification
- 📜 **NPTEL Elite Certificate** – Python for Data Science (IIT Madras, 77%)
- 🧠 Solved **200+ DSA problems** on LeetCode

<hr/>

<p align="center">
  ⭐ <i>Open to internships, entry-level roles, and impactful collaborations.</i> <br/>
  <b>Let's connect and build something awesome!</b>
</p>
